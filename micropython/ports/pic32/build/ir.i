...
void IRqueueSend(union IRpacket_u pkt)
{
    ...
    if ( ((IRpacketOutNext+1) % 8) != IRpacketOutCurr) {
...
void IRhandler()
{
    ...
    if (IRpacketInCurr != IRpacketInNext) {
        ...
        if (IRpacketsIn[IRpacketInCurr].p.address < IR_LASTADRESS)
...
void ir_ping(struct IRpacket_t p)
{
    ...
    if(p.data & 0x1000)
    {
        ...
        pkt.p.data = 0x2000 | 666;

        IRqueueSend(pkt);
    }

    else if(p.data & 0x2000)
    {
